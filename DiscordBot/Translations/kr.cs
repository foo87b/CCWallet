    ...
    public class kr : Catalog
    {
        public kr() : base(new CultureInfo("kr"))
        {
            Translations = new Dictionary<string, string[]>
            {
                ...
                {"Slight balance errors may occur due to network conditions.", new []{ "네트워크 상황에 따라 잔액에 약간의 에러가 있을 수 있습니다." } },
                ...
                {"It may take some time to receive an approved message from the network; you can also check the status with the Blockchain Explorer.", new []{ "네트워크 승인까지 시간이 더 소요됩니다. 블록체인 익스플로러를 확인해 주세요." } },
                ...
                {"Insufficient funds.", new []{ "지갑의 잔액이 부족합니다." } },
                {"Error - Transaction generation failed. {0}", new []{ "트랜잭션 생성에 실패했습니다. {0}" } },
                {"Error - Transaction transmission failed. {0}", new []{ "트랜잭션 송신에 실패했습니다. {0}" } },
